...
function GameSelection() {
 ...
 if(selected) return <Screen><TopBar title={selected.name}/>
   ...
   <button type="button" disabled={!affordable} onClick={()=>navigate({to:"/match",search:{game:selected.id,ladder:"start"}})} className="mt-3 w-full rounded-3xl border border-primary bg-primary/10 py-4 font-display text-sm font-bold tracking-[0.18em] text-primary disabled:opacity-30">∞ PLAY THE LADDER · €{wagerEur} DEMO</button>
