...
function GameSelection() {
 ...
 if(selected) return <Screen><TopBar title={selected.name} back="/games"/>
   ...
   <button type="button" disabled={!affordable||creating} onClick={async()=>{setCreating(true);try{const {challenge:ch,token}=await createFriendChallenge({gameId:selected.id,wagerEur,name:profile.username,avatar:profile.avatar});const url=`${window.location.origin}/challenge/${ch.code}`;sessionStorage.setItem("altameta:friendChallenge",JSON.stringify({code:ch.code,token,role:"creator",seed:ch.seed}));setShare({url,code:ch.code})}finally{setCreating(false)}}} className="mt-3 w-full rounded-3xl border border-primary/50 bg-card py-4 font-display text-sm font-bold tracking-[0.18em] text-primary disabled:opacity-30">{creating?"CREATING…":"CHALLENGE A FRIEND · 24H"}</button>
   {share&&<div className="mt-3 rounded-2xl border border-border bg-card p-4 text-center"><p className="text-[10px] uppercase tracking-[.2em] text-muted-foreground">Challenge code · valid for 24 hours</p><p className="mt-1 font-display text-2xl font-black tracking-[.2em] text-primary">{share.code}</p><button onClick={async()=>{if(navigator.share)await navigator.share({title:"ALTAMETA challenge",text:`Beat me at ${selected.name}!`,url:share.url});else await navigator.clipboard.writeText(share.url)}} className="mt-3 rounded-xl bg-primary px-5 py-3 text-xs font-bold text-primary-foreground">SHARE INVITE</button><button onClick={()=>navigate({to:"/challenge/$code",params:{code:share.code}})} className="ml-2 mt-3 rounded-xl border border-primary px-5 py-3 text-xs font-bold text-primary">PLAY NOW</button><p className="mt-2 break-all text-[10px] text-muted-foreground">{share.url}</p></div>}
 </Screen>;
