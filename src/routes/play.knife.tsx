...
import { impactAngle, hitsKnife } from "@/lib/duel/engine/knife";
...
function Knife() {
  ...
  const done = useRef(false),
    ...
    angle = useRef(0),
    scoreRef = useRef(0);
  const knives = useRef<number[]>([]),
    projectile = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    const tick = (t: number) => {
      ...
      last.current = t;
      angle.current =
        (angle.current + (105 + Math.min(220, scoreRef.current * 7)) * dt) %
        360;
      if (projectile.current !== null) {
        projectile.current = Math.min(1, projectile.current + dt * 7.5);
        if (projectile.current >= 1) {
          const hit = impactAngle(angle.current);
          if (hitsKnife(knives.current, angle.current)) {
            finish();
            return;
          }
          knives.current.push(hit);
          scoreRef.current++;
      ...
      if (projectile.current !== null) {
        const start = h - 38,
          ...
          yy = start + (end - start) * projectile.current;
        ctx.strokeStyle = "#eee";
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.moveTo(cx, yy);
  ...
  const throwKnife = () => {
    if (done.current || projectile.current !== null) return;
    ...
    <main className="mx-auto flex h-[100dvh] w-full max-w-md touch-none select-none flex-col overflow-hidden bg-background">
      ...
      >
        <OpponentOutBanner
          ...
          wagerEur={wagerEur}
          securedEur={
            ladder?.active && ladder.gameId === "knife"
