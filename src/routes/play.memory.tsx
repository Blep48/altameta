...
import {
  createMonkeyBoard,
  simulateMonkeyOpponent,
} from "@/lib/duel/engine/monkey";
...
function MonkeyTest() {
  ...
  const { activeMatch, finishMonkeyMatch, ready, profile, wagerEur } =
    useDuel();
  const [level, setLevel] = useState(0);
  const [next, setNext] = useState(1);
  const [covered, setCovered] = useState(false);
  const [phase, setPhase] = useState<
    "preview" | "sequence" | "playing" | "over"
  >("preview");
  const [highlighted, setHighlighted] = useState<number | null>(null);
  const finished = useRef(false);
  const seed = activeMatch?.seed ?? 1;
  const board = useMemo(() => createMonkeyBoard(seed, level), [seed, level]);
  const opponentLevels = useMemo(
    () =>
      activeMatch ? simulateMonkeyOpponent(seed, activeMatch.opponent) : 0,
    [seed, activeMatch],
  );
    ...
    timers.push(
      setTimeout(() => {
        ...
        board.forEach((tile, index) => {
          timers.push(
            setTimeout(() => setHighlighted(tile.number), index * 220),
          );
          timers.push(
            setTimeout(() => setHighlighted(null), index * 220 + 150),
          );
        });
        timers.push(
          setTimeout(
  ...
  const end = () => {
    ...
    if (activeMatch?.friend) {
      void submitIfFriend(activeMatch, level).then(() =>
        navigate({
          to: "/challenge/$code",
  ...
  return (
    <main className="mx-auto flex h-[100dvh] w-full max-w-md flex-col overflow-hidden overscroll-none bg-background">
      ...
      <header className="grid shrink-0 grid-cols-3 gap-2 px-5 pt-2 text-center">
        <Meter label="Level" value={String(level + 1)} />
        <Meter label="Tiles" value={String(board.length)} />
        <Meter label="Cleared" value={String(level)} />
      </header>
      <div className="px-5 pt-2 text-center text-xs text-muted-foreground">
        Memorize the numbers. After 1 second they flash in order, then
        disappear. Tap every square from 1 upward.
      </div>
      <section className="relative mx-4 mb-4 mt-3 min-h-0 flex-1 overflow-hidden rounded-3xl border border-border bg-card">
        <OpponentOutBanner
          ...
          opponentScore={opponentLevels}
          playerScore={level}
          wagerEur={wagerEur}
          outAfterMs={Math.max(1800, opponentLevels * 2600)}
          label="levels"
        />
        {board.map((tile) => (
          <button
            key={tile.number}
            type="button"
            onPointerDown={() => press(tile.number)}
            ...
            style={{
              ...
              width: "min(48px, calc(25% - 8px))",
              height: "min(48px, calc(22% - 8px))",
            }}
          >
            {!covered || tile.number < next ? tile.number : ""}
          </button>
        ))}
        {(phase === "preview" || phase === "sequence") && (
          <span className="absolute bottom-4 left-0 right-0 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-primary">
            {phase === "sequence" ? "WATCH THE ORDER" : "MEMORIZE"}
          </span>
        )}
        {phase === "over" && (
