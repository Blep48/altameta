import { DirectionField } from "@/components/duel/MovingField";
...
const SWIPE_MIN = 34;
const SYMBOL: Record<Direction, string> = {
  up: "↑",
  right: "→",
  down: "↓",
  left: "←",
...
function DirectionGame() {
  ...
  const { activeMatch, finishDirectionMatch, ready, profile, wagerEur } =
    useDuel();
  ...
  const end = (count: number) => {
    ...
    if (friend) {
      void submitIfFriend(match, count).then(() =>
        navigate({ to: "/challenge/$code", params: { code: friend.code } }),
      );
      return;
    }
    const outcome = finishDirectionMatch({
      playerArrows: count,
      opponentArrows: opponentOut,
    });
    setTimeout(() => {
      if (outcome) {
  ...
  useEffect(() => {
    ...
    const kickoff = setTimeout(() => {
      ...
      const tick = () => {
        ...
        if (t >= next.spawnMs + travelMs(next.index)) {
  ...
  const swipe = (direction: Direction) => {
    ...
    const next = chart[nextIndex.current];
    if (!next || performance.now() - startAt.current < next.spawnMs) return;
  ...
  const opponentOutMs =
    opponentOut < chart.length
      ? chart[opponentOut]!.spawnMs + travelMs(opponentOut)
      : chart[chart.length - 1]!.spawnMs + travelMs(chart.length - 1);
    ...
    <main className="mx-auto flex h-[100dvh] w-full max-w-md flex-col overflow-hidden overscroll-none bg-background">
      <div className="shrink-0 px-5 pt-3 text-center">
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
          Swipe in the direction shown by the lowest falling arrow. A wrong
          swipe or an arrow crossing the danger line ends the run.
        </p>
      </div>
      ...
      <header className="grid shrink-0 grid-cols-3 gap-2 px-5 pt-2 text-center">
        ...
        <Meter
          label="Opponent"
          value={
            destroyed >= opponentOut
              ? `OUT ${opponentOut}`
              : String(Math.min(destroyed, opponentOut))
          }
        />
      </header>
      ...
      >
        <OpponentOutBanner
          ...
          playerScore={destroyed}
          wagerEur={wagerEur}
          outAfterMs={1100 + opponentOutMs}
          label="arrows"
        />
        <div
          className="absolute inset-x-0 border-t-2 border-dashed border-destructive/70"
          style={{ top: `${DANGER_Y_PERCENT}%` }}
        >
          <span className="absolute right-3 -top-5 text-[9px] font-bold tracking-[0.2em] text-destructive">
            DANGER
          </span>
        </div>
        {phase === "playing" && (
        ...
        {phase === "ready" && (
          <Overlay
            title="GET READY"
            text="Swipe in the direction of the lowest arrow. One mistake and you're out."
          />
        )}
        {phase === "over" && (
          <Overlay
            title="OUT"
            text={`${destroyed} arrows · opponent ${opponentOut}`}
            danger
          />
        )}
        {flash === "hit" && (
      ...
      <div className="px-5 pb-3 pt-2 text-center text-xs text-muted-foreground">
        Swipe anywhere inside the arena. Destroy the lowest arrow before it
...
function Overlay({
  ...
  return (
    <div className="absolute inset-0 z-10 grid place-items-center bg-background/85 text-center">
      <div>
        <p
          className={`font-display text-3xl font-bold tracking-[0.28em] ${danger ? "text-destructive" : "text-primary"}`}
        >
          {title}
        </p>
        <p className="mt-2 px-8 text-sm text-muted-foreground">{text}</p>
      </div>
    </div>
  );
}
