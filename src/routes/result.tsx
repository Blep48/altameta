...
import { formatEuro, settlementAmount } from "@/lib/duel/economy";
import {
  ladderEliminations,
  ladderMultiplier,
  ladderPrizeUnits,
} from "@/lib/duel/ladder";
  ...
  head: () => ({
    meta: [
      { title: "Match result — DUEL" },
      {
        name: "description",
        content:
          "See how your reaction times stacked up against your opponent.",
      },
      { property: "og:title", content: "Match result — DUEL" },
      {
        property: "og:description",
        content: "Reaction duel results, coin and rating changes.",
      },
    ],
  }),
  component: Result,
...
function Result() {
  const { lastOutcome, profile, ready, canPlay, ladder, cashOutLadder } =
    useDuel();
  ...
  const ladderShownPrize =
    ...
    won &&
    ladder?.active &&
    ladder.gameId === lastOutcome.gameId
  ...
  useEffect(() => {
    ...
    const target = Math.max(
      ...
      ladderMatch
        ? ladderShownPrize
        : settlementAmount(true, lastOutcome.wagerEur),
    );
    ...
    const tick = (now: number) => {
      const duration = ladderMatch
        ? Math.min(
            4200,
            Math.max(2200, 1700 + Math.log10(Math.max(10, target)) * 420),
          )
        : Math.min(
            1800,
            Math.max(850, 700 + Math.log10(Math.max(10, target)) * 260),
          );
  ...
  useEffect(() => {
    ...
    let frame = 0;
    const tick = (now: number) => {
      const p = Math.min(1, (now - started) / 1600);
      const eased = 1 - Math.pow(1 - p, 4);
      setAnimatedCashout(Math.round(cashout.amount * eased));
  ...
  if (cashout) {
    const checkpoints = Array.from(
      { length: Math.min(cashout.streak, 12) },
      (_, i) => cashout.streak - Math.min(cashout.streak, 12) + i + 1,
    );
    const progress = cashout.amount
      ? Math.min(1, animatedCashout / cashout.amount)
      : 0;
    return (
      <Screen className="justify-center">
        <div className="animate-pop rounded-[2rem] border border-primary bg-primary/10 px-5 py-7 text-center neon-glow">
          <p className="text-[11px] font-black uppercase tracking-[.32em] text-primary">
            ∞ LADDER CASH OUT
          </p>
          <div className="mx-auto mt-6 flex h-64 max-w-xs items-stretch gap-4 text-left">
            <div className="relative w-5 shrink-0 rounded-full bg-secondary">
              <div
                className="absolute bottom-0 left-0 w-full rounded-full bg-primary transition-[height] duration-75"
                style={{ height: `${progress * 100}%` }}
              />
              {checkpoints.map((streak, i) => (
                <span
                  key={streak}
                  className="absolute left-1/2 h-3 w-3 -translate-x-1/2 rounded-full border border-primary bg-background"
                  style={{
                    bottom: `${((i + 1) / checkpoints.length) * 100}%`,
            ...
            <div className="relative flex-1">
              {checkpoints.map((streak, i) => {
                const reached = progress >= (i + 1) / checkpoints.length;
                return (
                  <div
                    key={streak}
                  ...
                  >
                    <span
                      className={`text-[10px] font-bold uppercase tracking-[.12em] ${reached ? "text-primary" : "text-muted-foreground"}`}
                    >
                      {ladderEliminations(streak)} players ·{" "}
                      {formatEuro(
                        Math.round(
                          lastOutcome.wagerEur * 100 * ladderMultiplier(streak),
                        ),
                      )}
                    </span>
                  </div>
          ...
          <p className="mt-5 text-[10px] uppercase tracking-[.2em] text-muted-foreground">
            {cashout.streak} wins · cash out secured
          </p>
          <p className="mt-2 break-all font-display text-4xl font-black tabular-nums text-primary text-glow">
            {formatEuro(animatedCashout)}
          </p>
        </div>
        <button
          type="button"
          onClick={() => navigate({ to: "/" })}
          className="mt-6 w-full rounded-2xl bg-primary py-5 font-display text-lg font-black tracking-[.22em] text-primary-foreground"
        >
          BACK TO HOME
        </button>
      </Screen>
    ...
    <Screen>
      ...
      {(rhythm || precision || direction || monkey || survival) && (
        <p className="mt-2 text-center text-[11px] uppercase tracking-[0.2em] text-muted-foreground">
          {survival
            ? lastOutcome.gameId === "knife"
              ? "Knives landed"
              : lastOutcome.gameId === "stack"
                ? "Blocks stacked"
          ...
          <ul className="mt-3 space-y-2">
            {lastOutcome.rounds.map((r) => (
              ...
              >
                ...
                <span
                  className={r.falseStart ? "text-destructive" : "text-primary"}
                >
          ...
          {ladder.active && won ? (
            <>
              ...
              <p className="mt-2 font-display text-4xl font-black text-primary">
                {ladderMultiplier(ladder.streak).toFixed(1)}×
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {ladderEliminations(ladder.streak)} players represented · DEMO
                prize {formatEuro(ladderPrizeUnits(ladder))}
              </p>
              <p className="mt-3 text-[11px] text-muted-foreground">
                Next win → {ladderMultiplier(ladder.streak + 1).toFixed(1)}× ·{" "}
                {formatEuro(
                  Math.round(
                    ladder.wagerEur * 100 * ladderMultiplier(ladder.streak + 1),
                  ),
                )}
              </p>
              <button
                ...
                className="mt-4 w-full rounded-2xl bg-primary py-4 font-display text-sm font-black tracking-[.16em] text-primary-foreground"
              >
                CONTINUE · RISK THE RUN
              </button>
              <button
                type="button"
                onClick={() => {
              ...
              >
                CASH OUT · {formatEuro(ladderPrizeUnits(ladder))}
              </button>
            </>
          ) : (
            <>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                The accumulated DEMO prize was lost.
              </p>
            </>
          )}
        </section>
