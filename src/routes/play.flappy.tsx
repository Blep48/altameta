...
function Flappy() {
  ...
  const bot = useMemo(
    () =>
      activeMatch
        ? simulateSurvivalOpponent(seed, activeMatch.opponent, "flappy")
        : 0,
    [seed, activeMatch],
  );
  ...
  const end = () => {
    ...
    const outcome = finishSurvivalMatch({
  ...
  useEffect(() => {
    ...
    const primary =
      getComputedStyle(arena).getPropertyValue("--primary").trim() || "#7c3aed";
    ...
    const tick = (now: number) => {
      ...
      for (const obstacle of feed) {
        ...
        const pipeWidth = Math.max(2.5, obstacle.size * 0.68),
          radiusX = (12 / size.width) * 100,
          radiusY = (12 / size.height) * 100;
        if (x + pipeWidth < 24 - radiusX) passed++;
        const gap = Math.max(18, obstacle.gap - Math.min(passed, 30) * 0.55);
        if (
          x < 24 + radiusX &&
          x + pipeWidth > 24 - radiusX &&
          (y.current - radiusY < obstacle.gapY - gap / 2 ||
            y.current + radiusY > obstacle.gapY + gap / 2)
        ) {
          end();
          return;
      ...
      for (const obstacle of feed) {
        ...
        const gap = Math.max(
          18,
          obstacle.gap - Math.min(scoreRef.current, 30) * 0.55,
        );
        const x = (w * xp) / 100;
    ...
    >
      ...
      >
        <OpponentOutBanner
          ...
          wagerEur={wagerEur}
          securedEur={
            ladder?.active && ladder.gameId === "flappy"
              ? ladderPrizeUnits(ladder) / 100
              : undefined
          }
          outAfterMs={Math.max(1500, bot * 1450)}
          label="pipes"
        />
        <canvas
          ref={canvasRef}
        ...
        />
        <p className="pointer-events-none absolute bottom-4 inset-x-0 text-center text-xs text-muted-foreground">
          TAP TO FLAP
        </p>
      </section>
    </main>
