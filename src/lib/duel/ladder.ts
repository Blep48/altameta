...
export const LADDER_PAYOUT_RATE=.95;
export const LADDER_FEE_RATE=1-LADDER_PAYOUT_RATE;
export const ladderGrossMultiplier=(streak:number)=>Math.pow(2,Math.max(0,streak));
export const ladderMultiplier=(streak:number)=>ladderGrossMultiplier(streak)*LADDER_PAYOUT_RATE;
export const ladderEliminations=(streak:number)=>ladderGrossMultiplier(streak)-1;
export const ladderPrizeUnits=(run:LadderRun)=>Math.round(run.wagerEur*100*ladderMultiplier(run.streak));
export const ladderFeeUnits=(run:LadderRun)=>Math.round(run.wagerEur*100*ladderGrossMultiplier(run.streak)*LADDER_FEE_RATE);
